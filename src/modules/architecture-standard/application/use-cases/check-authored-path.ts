...
export type AuthoringPathOperation = "write" | "delete" | "move-from" | "move-to";

function bounded(label: string, output: string): string { return `${label}: ${output.split("\n").slice(0, 40).join("\n")}`.trim(); }

export async function checkAuthoredPath(root: string, path: string, snapshotReader: RepositorySnapshotReader, commandRunner: CommandRunner, operation: AuthoringPathOperation = "write"): Promise<AuthoringCheckResult> {
  ...
  if (/\.(?:ts|tsx)$/.test(path)) {
    ...
    if (operation === "delete" || operation === "move-from") {
      return { path, checks: ["architecture"], violations: architecture.violations, commandFailures: [] };
    }
    ...
    return {
      path,
      checks: ["architecture", "lint", "typecheck"],
      violations: operation === "write" ? architecture.violations.filter((violation) => violation.path === path) : architecture.violations,
      commandFailures: outcomes.filter(({ result }) => result.exitCode !== 0).map(({ command, result }) => bounded(command[1] ?? command[0], result.output)),
    };
  }
