---
title: "WI1: Deterministic program issue graph checks and idempotent GitHub reconciliation"
...
program_work_item: abe862d6-b052-49fe-8611-bc1ab6e24253
work_item: 6a6a8bab-853f-4658-9bc0-38e2386b642d
...
# WI1: Deterministic Program Issue Graph Checks and Idempotent GitHub Reconciliation
...
- **Program:** Git-native work item `abe862d6-b052-49fe-8611-bc1ab6e24253`.
- **Work item:** Git-native work item `6a6a8bab-853f-4658-9bc0-38e2386b642d`, projected to GitHub issue `#25`.
- **Dependency:** U1A work item `745eda80-1e74-4866-bc95-2f2983b31025`. Rebase implementation on U1A's merged authoring and canonical-check behavior before editing.
...
- **Execution profile:** Use an isolated worktree, Bun only, no `any`, and a behaviorally meaningful failing test before each implementation unit.
- **Stop conditions:** Stop if the installed `git issue` version cannot provide stable git-native UUID and provider-mapping data without parsing human prose, if GitHub no longer exposes the required sub-issue APIs, or if reconciliation would have to delete or re-parent an unmanaged GitHub issue.
...
After this work, an operator can run one offline command to prove that every planned Mandem unit has one full git-native issue UUID and that plans, issues, dependencies, and authorization metadata agree. An explicitly named remote command will calculate the required GitHub changes, apply only managed changes when requested, and verify the result. Running the apply command again against the same state will report no changes and will not create another issue, milestone, label, comment, or parent-child link.

The workflow treats git-native UUIDs as portable identities. GitHub issue numbers are provider-specific mappings discovered from git-native issue history; they are never stored as the identity of a work item.

### Problem Frame

The 2026-07-28 program export required manual GitHub API calls after `git-native-issue` version 1.3.3 did not project later milestone and phase-label changes or native sub-issue hierarchy. The resulting GitHub graph is correct, but the commands that repaired it are absent from the repository and cannot be audited or repeated safely.

The child ExecPlans also use inconsistent identity metadata. Some contain no `work_item`, and U1A stores an abbreviated UUID. This prevents a program from proving that every planned unit has exactly one durable work item before execution begins.

### Requirements

#### Local relationship contract

- R1. Every program and child ExecPlan in the managed Mandem v1 plan set names its git-native work item with a complete lowercase UUID.
- R2. Every child ExecPlan names the program work item, its own work item, and all direct work-item dependencies in frontmatter fields with one versioned schema.
- R3. A versioned machine-readable metadata comment on each git-native issue owns its kind, unit, plan path when it represents a plan-backed unit, program UUID, direct parent UUID, and direct dependency UUIDs. The program issue's metadata also owns the provider repository, milestone, and managed label definitions. Plan frontmatter independently repeats portable relationship fields and must agree with the native issue.
- R4. The local checker rejects missing native issues, conflicting latest metadata comments, duplicate plan ownership, multiple native issues for one plan-backed unit, unknown dependencies, dependency cycles, paths outside the repository, nonexistent plan paths, malformed UUIDs, and disagreement between plan frontmatter and native issue metadata.
...
- R7. A read-only remote check compares each managed work item with its GitHub projection: provider mapping, open or closed state, direct parent, direct sub-issues, milestone, and the subset of labels owned by this workflow.
...
- R11. Reconciliation adds missing managed parent-child relationships and removes an unexpected relationship only when both issues belong to the native program graph and the desired metadata names the replacement. It fails closed before all writes for any unmanaged current parent or any unmanaged child attached to a managed parent.
...
- R15. `bun run issue-graph:check` runs offline and joins the canonical `bun run check` sequence. Developer setup merges native refs before work; CI fetches `refs/issues/*` explicitly before the gate. Missing program refs fail rather than silently skipping graph validation.
...
- A1. A plan author assigns a full git-native UUID to a child ExecPlan before seeking approval.
...
- F5. **New planned unit.** A contributor creates or selects a git-native issue first, appends its versioned relationship comment, then adds the same full UUID and dependencies to the child ExecPlan. The checker fails until both records agree.
...
- AE1. Given the checked-in Mandem v1 graph and no network access, when `bun run issue-graph:check` runs, it exits zero and prints a concise success line with the contract version and managed work-item count.
- AE2. Given a child plan with an abbreviated `work_item`, when the local check runs, it exits nonzero with the plan path and a stable malformed-UUID rule.
...
- AE4. Given a valid local graph and GitHub missing one parent-child relation, when remote preview runs, it prints one add-sub-issue operation and performs no writes.
...
- AE8. Given a child whose current GitHub parent is outside the managed graph, when apply runs, it fails with an unmanaged-parent diagnostic and does not re-parent the issue.
...
This unit manages the Mandem v1 program issue, its declared child work items, their provider projection, one named milestone, and a small declared label vocabulary. It does not mirror arbitrary repository issues, implement bidirectional merge semantics, replace `git-native-issue`, schedule work, or add Mandem runtime work-item behavior owned by U4.
...
The command preserves issue bodies, comments, assignees, projects, and labels outside the managed label set declared on the native program issue. It may close or reopen only provider issues whose native metadata assigns them to the program and whose unique Provider-ID mappings target the configured repository.
...
The program plan is `docs/plans/2026-07-21-001-feat-mandem-plan.md`. Its plan-backed units live below `docs/plans/units/`, and `docs/plans/units/README.md` records their sequence. The authoritative work ledger is stored in Git refs managed by `git-native-issue`; contributors use `git issue show`, `git issue merge`, `git issue sync`, and `git issue fsck` to inspect and exchange it.

On 2026-07-28, the program issue was projected to GitHub `#29`, its child units to issues `#21` through `#32`, and the current parent-child hierarchy was applied with GitHub's sub-issue API. The milestone and selected labels were also applied directly because the installed bridge did not update those fields on existing issues. These numbers are historical evidence only. The implementation must discover current numbers from provider records attached to each full git-native UUID.
...
The git-native issue refs are the authoritative program graph. Each managed issue receives an append-only comment whose first line is exactly `Mandem-Graph-Metadata: v1`; the remaining comment is YAML with only `kind`, `unit`, `program`, `plan`, `parent`, `depends_on`, `provider`, `milestone`, and `managed_labels`. `provider`, `milestone`, and `managed_labels` are nested objects valid only on the program issue. A support issue uses `plan: null`. Parsing rejects unknown keys.
...
Plan frontmatter duplicates the unit's UUID, program UUID, and dependencies so an ExecPlan remains self-contained. The checker compares that declaration with its authoritative native issue. When the two disagree, it fails; it never rewrites the native issue from the plan. Relationship changes begin with a new native issue metadata comment, followed by matching plan edits.

During implementation, append the reviewed metadata below to the program, U1, U1C, U1A, its incident, WI1, and U2-U10 issue refs. Then edit each plan to use complete `program_work_item`, `work_item`, and `depends_on_work_items`. Add WI1 as a dependency of U2 because the checker must join the canonical repository gate before the next runtime unit can receive execution authorization. Update U2's plan and human status comment after its authoritative metadata.

The exact plan-frontmatter contract for a child is:

    program_work_item: <full lowercase UUID>
    work_item: <full lowercase UUID>
    depends_on_work_items:
      - <full lowercase UUID>

The program plan uses `work_item` for its own full UUID and an empty `depends_on_work_items` array. A supporting incident has no plan and therefore appears only in native issue metadata. A plan-backed unit has exactly one plan.

The native setter accepts YAML without the comment marker. A plan-backed child payload has exactly this shape:

    kind: "unit"
    unit: "U2"
    program: "abe862d6-b052-49fe-8611-bc1ab6e24253"
    plan: "docs/plans/units/u2-protocol-lifecycle-sqlite.md"
    parent: "abe862d6-b052-49fe-8611-bc1ab6e24253"
    depends_on:
      - "6a6a8bab-853f-4658-9bc0-38e2386b642d"
      - "745eda80-1e74-4866-bc95-2f2983b31025"
      - "da645bd0-9899-40b3-9f23-3b48d65362a4"

A support issue uses the same child keys with `kind: "support"` and `plan: null`:

    kind: "support"
    unit: "U1A-INCIDENT-1"
    program: "abe862d6-b052-49fe-8611-bc1ab6e24253"
    plan: null
    parent: "745eda80-1e74-4866-bc95-2f2983b31025"
    depends_on: []

The program payload has exactly this nesting:
    ...
    milestone:
      title: "Mandem v1"
      description: "Tracks the planned U1–U10 program through final v1 publication."
      state: "open"
      due_on: null
    managed_labels:
      blocked:
      ...
      incident:
        color: "D93F0B"
        description: "Operational or reliability incident requiring corrective work."
      planned:
        color: "D4C5F9"
      ...
      u1a:
        color: "C5DEF5"
        description: "Mandem program unit U1A."
      u1c:
        color: "C5DEF5"
        description: "Mandem program unit U1C."
      u2:
        color: "ededed"
        description: ""
      u3:
...
Program metadata requires `provider`, `milestone`, and the complete `managed_labels` object shown above; child and support metadata reject those keys. Every payload requires the six relationship keys shown for its kind. `parent` must be null only for the program. `plan` must be null only for support. `depends_on` may be empty.
...
Allowed `promotion` values are `scaffolded`, `planned`, `clean-room-approved`, `executable`, and `complete`. `execution_authorized: true` is valid only with `promotion: executable`. A completed unit uses `promotion: complete` and `execution_authorized: false`. Normalize U1A from its current `clean-room-approved` plus `true` combination to `executable` plus `true`; this changes stale metadata, not its already recorded authority.

The reviewed native issue migration set follows. Each line identifies the target ref and its complete relationship values. `state` and `labels` refer to the git-native issue's built-in fields; the relationship comment does not duplicate them. The program comment additionally contains the nested `provider`, `milestone`, and complete `managed_labels` objects from the canonical program payload above.

      - { unit: PROGRAM, kind: program, work_item: abe862d6-b052-49fe-8611-bc1ab6e24253, program: abe862d6-b052-49fe-8611-bc1ab6e24253, plan: docs/plans/2026-07-21-001-feat-mandem-plan.md, parent: null, depends_on: [], state: open, labels: [in-progress] }
      - { unit: U1, kind: unit, work_item: da645bd0-9899-40b3-9f23-3b48d65362a4, program: abe862d6-b052-49fe-8611-bc1ab6e24253, plan: docs/plans/units/u1-bootstrap-repository-architecture-contract.md, parent: abe862d6-b052-49fe-8611-bc1ab6e24253, depends_on: [], state: closed, labels: [] }
      - { unit: U1C, kind: unit, work_item: 5717221b-f9e6-4c8f-abca-77a1ad3811bf, program: abe862d6-b052-49fe-8611-bc1ab6e24253, plan: docs/plans/units/u1-corrective-architecture-package-contract.md, parent: da645bd0-9899-40b3-9f23-3b48d65362a4, depends_on: [da645bd0-9899-40b3-9f23-3b48d65362a4], state: closed, labels: [u1c] }
      - { unit: U1A, kind: unit, work_item: 745eda80-1e74-4866-bc95-2f2983b31025, program: abe862d6-b052-49fe-8611-bc1ab6e24253, plan: docs/plans/units/u1a-documentation-authoring-quality-gates.md, parent: da645bd0-9899-40b3-9f23-3b48d65362a4, depends_on: [5717221b-f9e6-4c8f-abca-77a1ad3811bf], state: open, labels: [in-progress, u1a] }
      - { unit: U1A-INCIDENT-1, kind: support, work_item: 38f956c8-0f18-4d85-af1a-d908bcc54248, program: abe862d6-b052-49fe-8611-bc1ab6e24253, plan: null, parent: 745eda80-1e74-4866-bc95-2f2983b31025, depends_on: [], state: open, labels: [incident] }
      - { unit: WI1, kind: unit, work_item: 6a6a8bab-853f-4658-9bc0-38e2386b642d, program: abe862d6-b052-49fe-8611-bc1ab6e24253, plan: docs/plans/units/wi1-program-issue-graph-integrity.md, parent: abe862d6-b052-49fe-8611-bc1ab6e24253, depends_on: [745eda80-1e74-4866-bc95-2f2983b31025], state: open, labels: [planned] }
      - { unit: U2, kind: unit, work_item: cb67d131-975c-4d97-9a6f-4934be991ac6, program: abe862d6-b052-49fe-8611-bc1ab6e24253, plan: docs/plans/units/u2-protocol-lifecycle-sqlite.md, parent: abe862d6-b052-49fe-8611-bc1ab6e24253, depends_on: [da645bd0-9899-40b3-9f23-3b48d65362a4, 745eda80-1e74-4866-bc95-2f2983b31025, 6a6a8bab-853f-4658-9bc0-38e2386b642d], state: open, labels: [blocked, u2] }
      - { unit: U3, kind: unit, work_item: d946e066-84d5-4651-b3b4-30a18e80008c, program: abe862d6-b052-49fe-8611-bc1ab6e24253, plan: docs/plans/units/u3-server-docker-resident-reconciliation.md, parent: abe862d6-b052-49fe-8611-bc1ab6e24253, depends_on: [cb67d131-975c-4d97-9a6f-4934be991ac6], state: open, labels: [blocked, u3] }
      - { unit: U4, kind: unit, work_item: 11538b56-bd63-42c2-8242-87ac7a76d35d, program: abe862d6-b052-49fe-8611-bc1ab6e24253, plan: docs/plans/units/u4-work-items-plans-queue-gates-cli.md, parent: abe862d6-b052-49fe-8611-bc1ab6e24253, depends_on: [cb67d131-975c-4d97-9a6f-4934be991ac6, d946e066-84d5-4651-b3b4-30a18e80008c], state: open, labels: [blocked, u4] }
      - { unit: U5, kind: unit, work_item: cf239716-00e2-46ae-82e7-84ac8f31baaf, program: abe862d6-b052-49fe-8611-bc1ab6e24253, plan: docs/plans/units/u5-operating-docs-provider-sessions.md, parent: abe862d6-b052-49fe-8611-bc1ab6e24253, depends_on: [d946e066-84d5-4651-b3b4-30a18e80008c, 11538b56-bd63-42c2-8242-87ac7a76d35d], state: open, labels: [blocked, u5] }
      - { unit: U6, kind: unit, work_item: 22a35ab0-878b-448b-9341-6679b70a499d, program: abe862d6-b052-49fe-8611-bc1ab6e24253, plan: docs/plans/units/u6-unattended-work-review-learn-merge.md, parent: abe862d6-b052-49fe-8611-bc1ab6e24253, depends_on: [cb67d131-975c-4d97-9a6f-4934be991ac6, d946e066-84d5-4651-b3b4-30a18e80008c, 11538b56-bd63-42c2-8242-87ac7a76d35d, cf239716-00e2-46ae-82e7-84ac8f31baaf], state: open, labels: [blocked, u6] }
      - { unit: U7, kind: unit, work_item: 6ca36caa-37e2-447b-935e-792a6f6566b6, program: abe862d6-b052-49fe-8611-bc1ab6e24253, plan: docs/plans/units/u7-complete-cli-toon-opentui.md, parent: abe862d6-b052-49fe-8611-bc1ab6e24253, depends_on: [cb67d131-975c-4d97-9a6f-4934be991ac6, d946e066-84d5-4651-b3b4-30a18e80008c, 11538b56-bd63-42c2-8242-87ac7a76d35d, cf239716-00e2-46ae-82e7-84ac8f31baaf, 22a35ab0-878b-448b-9341-6679b70a499d], state: open, labels: [blocked, u7] }
      - { unit: U8, kind: unit, work_item: 9e6cde19-27d2-4228-8a93-628829ae1b92, program: abe862d6-b052-49fe-8611-bc1ab6e24253, plan: docs/plans/units/u8-sbp-install-architecture-baseline.md, parent: abe862d6-b052-49fe-8611-bc1ab6e24253, depends_on: [da645bd0-9899-40b3-9f23-3b48d65362a4, cb67d131-975c-4d97-9a6f-4934be991ac6, d946e066-84d5-4651-b3b4-30a18e80008c, 11538b56-bd63-42c2-8242-87ac7a76d35d, cf239716-00e2-46ae-82e7-84ac8f31baaf, 22a35ab0-878b-448b-9341-6679b70a499d, 6ca36caa-37e2-447b-935e-792a6f6566b6], state: open, labels: [blocked, u8] }
      - { unit: U9, kind: unit, work_item: 7351af1b-d406-4768-bbf9-21f878aad28a, program: abe862d6-b052-49fe-8611-bc1ab6e24253, plan: docs/plans/units/u9-restart-proof-sbp-release-candidate.md, parent: abe862d6-b052-49fe-8611-bc1ab6e24253, depends_on: [da645bd0-9899-40b3-9f23-3b48d65362a4, cb67d131-975c-4d97-9a6f-4934be991ac6, d946e066-84d5-4651-b3b4-30a18e80008c, 11538b56-bd63-42c2-8242-87ac7a76d35d, cf239716-00e2-46ae-82e7-84ac8f31baaf, 22a35ab0-878b-448b-9341-6679b70a499d, 6ca36caa-37e2-447b-935e-792a6f6566b6, 9e6cde19-27d2-4228-8a93-628829ae1b92], state: open, labels: [blocked, u9] }
      - { unit: U10, kind: unit, work_item: 237397e3-cf06-4c6f-bf5d-ce83d8187ee3, program: abe862d6-b052-49fe-8611-bc1ab6e24253, plan: docs/plans/units/u10-observability-final-v1.md, parent: abe862d6-b052-49fe-8611-bc1ab6e24253, depends_on: [7351af1b-d406-4768-bbf9-21f878aad28a], state: open, labels: [blocked, u10] }

The program issue's `managed_labels` field defines these exact reviewed values. Colors omit the leading `#`; an empty quoted string is intentional.
    ...
    incident: { color: D93F0B, description: "Operational or reliability incident requiring corrective work." }
    u1c: { color: C5DEF5, description: "Mandem program unit U1C." }
    u1a: { color: C5DEF5, description: "Mandem program unit U1A." }
...
The milestone contract is title `Mandem v1`, description `Tracks the planned U1–U10 program through final v1 publication.`, open state, and no due date. Labels outside the managed set are preserved.

The provider projection derives desired issue state from each native issue's built-in `State` trailer and desired managed-label assignments from the intersection of its built-in `Labels` trailers with the program issue's `managed_labels` keys. GitHub fields never flow back into these native values through this reconciler.

The initial diagnostic catalog is `IGRAPH-NATIVE-METADATA`, `IGRAPH-NATIVE-CONFLICT`, `IGRAPH-UUID`, `IGRAPH-PATH`, `IGRAPH-PLAN-OWNER`, `IGRAPH-ISSUE-MISSING`, `IGRAPH-DEPENDENCY`, `IGRAPH-CYCLE`, `IGRAPH-FRONTMATTER`, `IGRAPH-AUTHORIZATION`, `IGRAPH-PROVIDER-MAPPING`, `IGRAPH-PROVIDER-STATE`, `IGRAPH-PROVIDER-LABEL`, `IGRAPH-PROVIDER-MILESTONE`, `IGRAPH-PROVIDER-PARENT`, `IGRAPH-PROVIDER-UNMANAGED-PARENT`, and `IGRAPH-PROVIDER-UNMANAGED-CHILD`. Findings sort by rule ID, work-item UUID, plan path, then message.
...
- KTD2. **Use native issue metadata plus plan cross-checks.** Git-native refs own program discovery and provider policy. Plan frontmatter repeats portable relationships and is compared to detect stale edits. Checked-in plans and GitHub never override native metadata.
...
### U1. Define and validate the native relationship contract
...
- **Test scenarios:** Begin with failing tests for malformed and abbreviated UUIDs; incomparable latest metadata comments; duplicate plan ownership; multiple issues for one plan-backed unit; missing program, dependency, issue, and plan; mismatched dependencies; path traversal; dependency cycles; invalid authorization and promotion combinations; unknown metadata keys; required and forbidden fields for program, unit, and support kinds; canonical field order; sorted dependency UUIDs and label names; CRLF normalization to LF with one final newline; semantic equality across differently formatted YAML; and stable finding order. Add a complete valid Mandem v1 fixture that returns no findings.
...
### U2. Read plans and git-native issues through application ports
...
### U3. Normalize the current Mandem v1 graph and add the offline command

- **Goal:** Make the checked-in program graph satisfy the new contract and include the local check in the repository gate.
- **Files:** Update the managed plan frontmatter in `docs/plans/2026-07-21-001-feat-mandem-plan.md` and `docs/plans/units/*.md`; update `docs/plans/units/README.md`; append authoritative metadata comments to all managed git-native issues; create `src/modules/architecture-standard/application/use-cases/set-native-issue-graph-metadata.ts`, `src/modules/architecture-standard/api/issue-graph.ts`, `src/modules/architecture-standard/api/issue-graph.test.ts`, `scripts/check-issue-graph.ts`, `scripts/set-issue-graph-metadata.ts`, and their tests; update the API and public module barrels, `package.json`, U1A's `.github/workflows/repository-quality.yml`, and documentation indexes required by U1A.
- **Approach:** Compose the filesystem and git-native adapters with the application use cases in the API module. Keep scripts limited to argument parsing, rendering, and exit status. The setter fetches the exact remote issue ref into a temporary ref and applies the ancestry state machine in `Idempotence and Recovery`; it never appends on a stale local head. Only `--apply` may append through `git issue comment` or push the exact existing local ref. Use the setter to apply the reviewed native metadata set above, then normalize plan frontmatter to match. Add `issue-graph:check` before typecheck in the canonical `check` sequence. Before the CI gate, add `git fetch origin '+refs/issues/*:refs/issues/*'` to `.github/workflows/repository-quality.yml`; the quoted refspec prevents shell glob expansion.
- **Test scenarios:** Begin with a failing disposable-repository test that creates plan files and issue refs, then proves success, malformed-plan failure, missing-issue failure, and deterministic output. Verify the setter preview performs no commit or push; first apply appends and pushes one ref; a local-behind remote ref fails without append; divergent refs fail without append; a merged metadata conflict appends one resolving canonical comment even when one conflicting payload matches the request; retry after a lost push performs zero commits and one push only when the remote-to-local range is exactly the expected metadata commit; an extra unpushed comment causes zero pushes and an operator-review error; and the next repeated apply performs zero commits and zero pushes. Verify `bun run issue-graph:check` does not invoke `gh`, fetch, merge, sync, or push. Verify the CI workflow fetches issue refs before `bun run check` and the current repository passes.
...
### U4. Plan GitHub reconciliation without mutation
...
### U6. Document the contract and integrate final verification
...
Milestone 1 implements U1 and U2. It ends when a pure test fixture and a disposable Git repository both prove that the reviewed v1 graph either validates or returns stable rule IDs. No GitHub process runs during this milestone.

Milestone 2 implements U3. It appends the reviewed metadata to native issue refs, pushes each ref, normalizes every plan's frontmatter against those native records, and connects the offline command to the canonical check through the API composition layer. It ends when `bun run issue-graph:check` and the full offline gate pass without credentials.

Milestone 3 implements U4. It translates provider snapshots into a stable operation list without mutation. It ends when tests cover every managed provider field, missing mappings, unmanaged parents, and ordering independence.

Milestone 4 implements U5. It adds the GitHub adapter, preview, explicit apply, immediate re-read behavior, and zero-write retry proof. It ends when fake-provider interruption tests pass and the live remote check reports the actual repository state without changing it.

Milestone 5 implements U6 and the shipping tail. It completes indexed documentation, runs live apply twice, captures the required zero-write evidence from the second run, runs every gate, pushes the implementation branch, and opens a pull request.
...
For each unit, add the named failing test first and run only that test file. Preserve the failing output in this plan's `Artifacts and Notes`, implement the smallest behavior, then rerun the focused test. After U3, run:
...
Expect the local command to print one line in this form and exit zero:
    ...
    issue graph native v1 valid: 15 managed work items

The managed count is fifteen: the program, thirteen plan-backed child units including WI1, and one U1A support incident. If the reviewed native metadata set changes before authorization, revise this plan and repeat review instead of silently changing the expected value.

After U4 and U5 tests pass, preview the provider comparison:

    bun run issue-graph:remote:check

An aligned provider prints `issue graph remote valid: 15 managed work items, 0 operations` and exits zero. Drift prints the sorted operations, performs no writes, and exits one. Operational failures such as missing authentication or an ambiguous provider mapping exit two.
...
The first command must finish with zero post-check drift. The second must print `issue graph remote valid: 15 managed work items, 0 operations, 0 writes`. Run both even when the initial preview is already aligned; in that case both apply invocations must perform zero writes.
...
State changes use a field-specific issue patch. Label reconciliation adds or removes only labels named in the native program issue's `managed_labels`. Milestone reconciliation selects the one milestone whose exact title matches native program metadata. Hierarchy reconciliation removes a current parent only after proving both current and desired parents are managed. An unmanaged child attached to a managed parent produces `IGRAPH-PROVIDER-UNMANAGED-CHILD` before any write; the operator must decide that external relationship separately. These rules preserve provider state outside the reviewed contract.
...
The first manual reconciliation established these provider relationships: program issue `#29` contains direct sub-issues `#21`, `#22` through `#28`, and `#30` through `#32`; U1 issue `#21` contains U1C `#18` and U1A `#20`; U1A `#20` contains incident `#17`. These numbers are evidence for the current repository only and do not appear in plan identity metadata.
...
The domain layer defines `NativeIssueGraphV1`, `IssueGraphNode`, `IssueGraphFinding`, `ProviderIssueState`, and a discriminated `IssueGraphOperation` union. The application layer exposes these interfaces:

    interface LocalIssueGraphRepository {
      listIssueRefs(): Promise<readonly string[]>;
      readIssue(workItem: string): Promise<LocalIssueRecord | null>;
...
Use `unknown` only at untrusted parse boundaries and narrow it before domain construction. Do not use `any`. `LocalIssueRecord` includes full UUID, local open/closed state, and zero or more provider mappings. `ProviderMapping` includes provider kind, owner, repository, and provider issue number as an adapter value, never as portable identity. `IssueGraphOperation` includes create-or-update managed label, create-or-update milestone, set managed issue state, add managed label, remove managed label, set milestone, add sub-issue, and move managed sub-issue.
...
- `GET /repos/{owner}/{repo}/labels` lists definitions. `POST /repos/{owner}/{repo}/labels` creates a missing definition. `PATCH /repos/{owner}/{repo}/labels/{name}` updates only the native-program-policy color and description.
...
- `GET /repos/{owner}/{repo}/issues/{issue_number}/parent` reads the direct parent; a not-found response means no parent. `GET /repos/{owner}/{repo}/issues/{issue_number}/sub_issues` lists direct children.
- `POST /repos/{owner}/{repo}/issues/{parent_number}/sub_issues` with `{"sub_issue_id": <provider issue database id>}` adds a direct child. `DELETE /repos/{owner}/{repo}/issues/{parent_number}/sub_issue` with the same payload removes it before a permitted managed move. The fake-adapter test asserts this exact singular delete path and payload.
...
- Native `Mandem-Graph-Metadata: v1` comments include the program, WI1, U1, U1C, U1A, its support incident, and U2-U10 with full UUID relationships, and every exact issue ref was pushed.
...
- The live GitHub projection has the native issue states, declared direct hierarchy, milestone, and managed labels, while all unrelated provider data remains intact; unmanaged children under managed parents fail closed before writes.
...
- [x] (2026-07-28 20:26Z) Drafted this implementation-ready child ExecPlan with explicit idempotence, retry, and ownership rules.
...
- [x] (2026-07-28 20:47Z) Completed clean-room, coherence, and feasibility review and resolved every blocking finding.
- [ ] Obtain operator approval of that exact reviewed revision.
- [ ] Change only authorization metadata to `promotion: executable` and `execution_authorized: true`.
- [ ] Rebase an isolated implementation worktree on merged U1A and complete U1-U6 in order.
...
- Observation: The installed CLI has no local command for native parent or dependency relationships, while GitHub exposes parent and sub-issue relationships separately.
...
- Observation: Current child-plan identity metadata is incomplete or abbreviated.
  Evidence: U1A contains `work_item: 745eda8`, while multiple U2-U10 scaffolds contain no `work_item`.
...
- Decision: Require a second zero-write apply as acceptance evidence.
  Rationale: Unit tests prove modeled idempotence; the repeated live command proves the adapter observes the real provider state consistently.
  Date/Author: 2026-07-28 / Codex

## Outcomes & Retrospective

Planning is complete but not yet reviewed or authorized. No implementation or provider mutation is authorized by this revision.
...
Revision note (2026-07-28): Clarified authority after operator feedback. Versioned comments in native issue refs now own the graph and provider policy; plan frontmatter is a checked cross-reference, and GitHub is only a projection. Removed the hand-edited manifest design, added deterministic conflict handling for concurrent metadata comments, covered state and unmanaged-child planning, and corrected GitHub's singular sub-issue removal endpoint.
