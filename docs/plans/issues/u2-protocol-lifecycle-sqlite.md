...
Lease creation is also deterministic. A new lease uses the accepted command ID as `lease_id`, the command timestamp as `acquired_at` and initial `last_heartbeat_at`, and the prior resource's fencing token plus one, or `1` when the issue has never held that resource. `acquire-work-lease` creates a `work` resource; `accept-learn` creates an `integration` resource; review repair creates a `work` resource. Takeover revokes the prior lease at the takeover command timestamp, records `takeover`, and creates the replacement with the takeover command ID and next token in one event payload. Heartbeat records a fact but never moves expiry. A release from `Working` sets `revoked_at` to the command timestamp, records the bounded operator summary and reconciliation artifacts in the event, clears the active lease, and moves to `Queued`; only then may `acquire-work-lease` dispatch a fresh worker. `Merging` rejects `release-work-lease` because an external transaction may have started. Its takeover owner must use `record-exact-merge`, `return-for-repair`, or `record-reconciliation-conflict` after reading Git and provider evidence. A released lease cannot heartbeat, hand off, merge, or release again; those attempts fail `LEASE_REQUIRED` or `LEASE_FENCED` according to the ordered error catalog. SQLite stores the complete lease value, and replay reconstructs it only from lease-bearing events.
...
| `accept-plan-review` | `review_session_id`, `plan`, `governing_contract`, `review_output`, `review_evidence`, `verdict: "executor-safe"` |
...
Every artifact-valued field is `ArtifactReferenceV1`; every evidence-valued field is a nonempty bounded array of `ArtifactReferenceV1`. `blocker_codes` is a nonempty bounded array of `ErrorCodeV1`. `reviewer_provenance` uses the complete `ReviewerProvenanceV1` shape below. Lease fencing tokens are decimal strings that parse to positive 64-bit integers. Destination uses the complete `CheckpointDestinationV1` shape below.
...
The named nested values have these complete shapes. No command may substitute free-form JSON for them.
    ...
    interface ReviewEvidenceTargetV1 { path: RepoPath; commit: GitSha; digest: Sha256; }
    ...
    interface ReviewerProvenanceV1 { reviewer: ReviewSessionIdentityV1; authors: ReviewSessionIdentityV1[]; received_authoring_context: false; wrote_only_manifest_output: true; challenge_lenses: ("plans-conformance" | "executor-safety" | "counterexample" | "security" | "feasibility")[]; risk_policy: ReviewerRiskPolicyV1; }
    interface ReviewWriteV1 { path: RepoPath; digest: Sha256; }
    interface ReviewEvidenceBundleV1 { manifest: ReviewManifestTargetV1; output: ReviewOutputTargetV1; reviewer_commit: GitSha; write_set: ReviewWriteV1[]; reviewer: ReviewSessionIdentityV1; authors: ReviewSessionIdentityV1[]; revisers: ReviewSessionIdentityV1[]; received_authoring_context: boolean; risk_policy: ReviewerRiskPolicyV1; source_artifacts: ArtifactReferenceV1[]; }
    interface ValidatedReviewEvidenceV1 { bundle_digest: Sha256; manifest: ReviewManifestTargetV1; output: ReviewOutputTargetV1; reviewer_commit: GitSha; sole_write: ReviewWriteV1; reviewer: ReviewSessionIdentityV1; authors: ReviewSessionIdentityV1[]; revisers: ReviewSessionIdentityV1[]; risk_policy: ReviewerRiskPolicyV1; source_artifacts: ArtifactReferenceV1[]; }
...
`EventPayloadV1` uses `kind` as its discriminator. The transition table maps each lifecycle command to exactly one past-tense event kind. Every such payload has exactly `{ kind, from_state, to_state, command_payload, lease_change }`: `command_payload` is the complete accepted `CommandPayloadV1` variant, including its original discriminator; `lease_change` is `null` unless the transition changes a lease, otherwise it is the complete resulting `LeaseSnapshotV1`. Portable transitions then emit `portable-checkpoint-requested` with exactly `{ kind, checkpoint_id, originating_event_id, payload_digest, destination }`. This reuse of the closed command variant is the event's complete governed data shape; an implementation may not flatten, omit, or add fields. The state-preserving families use these exact forms:
...
`evaluateLifecycleCommand(input: LifecyclePolicyInputV1): LifecyclePolicyDecisionV1` is the only lifecycle reducer entry point. It reads no clock, database, Git, provider, or ambient working-tree state. Application code builds the input from the validated envelope, trusted principal, `EventStorePort.loadIssue`, and validated review evidence when the command accepts a review. All other commands require `validated_review_evidence: null`. Replay applies the recorded event payloads and never reevaluates historical freshness or risk policy. Parsers, serializers, fixtures, and both root barrels include every policy and lease value above.
    ...
    interface ReviewEvidencePort { loadBoundEvidence(target: ReviewEvidenceTargetV1): Promise<ReviewEvidenceBundleV1>; }
...
`ReviewEvidencePort` is application-owned and trust-bearing. Its Git-backed adapter reads the manifest and output at their exact commits, obtains the reviewer commit's parent-to-commit write set, and reads the committed orchestrator evidence artifact named by `ReviewEvidenceTargetV1`. That artifact records separate author, reviser, and reviewer identities; whether authoring context was inherited; provider and model facts; availability evidence; and the manifest-declared sole output path. Provider session attestations, when available, are artifact references in `source_artifacts`; the command envelope cannot override them. The adapter returns canonical sorted writes and identities. `validateReviewEvidence(bundle, currentPlan, currentContract): ValidatedReviewEvidenceV1 | ProtocolErrorV1` rereads and hashes the manifest and output bytes, requires exactly one write equal to the manifest output path, rejects reviewer identity in either authors or revisers, rejects inherited context, applies the risk rule, and binds the validated bundle digest. Terminal output has no artifact and therefore returns `ARTIFACT_MISSING`; extra writes or identity/context mismatch return `ACTOR_ATTRIBUTION_MISMATCH`; stale bytes or availability evidence return `ARTIFACT_STALE`. `executeCommand` must call this validator before `evaluateLifecycleCommand` for plan-review acceptance and must store the validated value in `ReviewDecisionV1` and its event.
...
- **Test scenarios:**
  ...
  10. Review evidence rejects terminal-only output, extra writes, author or reviser self-review, inherited authoring context, stale availability evidence, and a required-but-unused alternative. It accepts and stores `required-and-used` and properly evidenced `unavailable-permitted` cases.
...
- **Concrete red/green proof:** Create the seven port interfaces and their in-memory fakes first. Then add tests named `returns stored bytes after a lost response`, `blocks a successor while checkpoint evidence is pending`, `observes before writing a checkpoint`, `reads plan bytes from the approved commit`, `derives review acceptance from committed evidence`, and `replaces projections only after anchor verification`. Add the terminal-only, extra-write, author, reviser, inherited-context, risk-availability, and stale-evidence cases to `validate-review-evidence.test.ts`; add `reads exact review bytes and sole commit write set` against a disposable Git repository in `git-review-evidence.test.ts`. Run `bunx vitest run src/modules/execution/application src/modules/runtime/infrastructure/repositories/git-review-evidence.test.ts`; it must fail because `GitReviewEvidenceRepository`, `validateReviewEvidence`, `executeCommand`, `completeCheckpoint`, and `rebuildProjections` are absent. Implement those named exports, re-export application contracts through their root barrels without exporting the Git adapter, and rerun the command until every test passes. Run `bun run typecheck` afterward to prove each fake and adapter satisfies the public port without a cast.
