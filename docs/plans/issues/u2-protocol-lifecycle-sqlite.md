...
- AE12. Given a reviewer returns findings in terminal output but does not write the manifest's output file, when review acceptance is requested, then Mandem rejects it without a verdict event. Given the reviewer writes only the named file, Mandem preserves those exact bytes and digest; a later synthesis uses a different path and names its source digest and transformation.
...
| `PlanReview` | `accept-plan-review` | phase agent | Matching review session | Full `PLANS.md` conformance and executor-safe verdict from the exact reviewer-authored output path and digest; write set contains only that output; plan and governing-contract targets remain unchanged; every process finding has one terminal disposition | `NeedsApproval` | Review accepted; completed | portable review verdict |
...
- **Requirements:** R5-R12b, R21-R23; AE4, AE5, AE9-AE12.
...
- **Test scenarios:**
  ...
  10. Review acceptance rejects a missing output file, terminal-only result, orchestrator-authored substitute, or reviewer write outside the manifest path. An exact sole-file write preserves the reviewer bytes and digest; a synthesis must use a distinct source-linked artifact.
  11. `Done` rejects zero, duplicate-current, unresolved, or conflicting dispositions and accepts exactly one current terminal disposition per routed item.
  12. The transition-fixture inventory has exact row and deterministic-order parity with the catalog.
