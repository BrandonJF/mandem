...
- **Test scenarios:**
  ...
  13. Stop after dispatch commits to SQLite but before the receipt artifact write. Restart observes the one derived receipt path, writes the exact outbox bytes, records the returned commit target in `ValidatedCheckpointEvidenceV1`, and permits acceptance. Matching preexisting bytes succeed; conflicting bytes at that path enter checkpoint conflict. Repeat for an issue-ref checkpoint. Then delete every checkpoint and lifecycle projection, rebuild only from events without calling `PortableCheckpointPort`, and prove byte-identical verified targets, checkpoint states, dispatch receipt target, and review-acceptance behavior for both destination kinds.
