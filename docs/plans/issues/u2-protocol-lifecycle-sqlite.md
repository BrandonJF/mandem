...
- KTD18. **Separate manifest acceptance, provider dispatch, and verdict acceptance.** Commit and checkpoint the complete pre-dispatch manifest before any provider launch. Dispatch then observes or launches one deterministic identity, stores the provider receipt, and checkpoints it. Verdict acceptance uses one exact reviewer commit as an untrusted locator and derives every accepted fact from its bytes and bound evidence. This order removes the manifest/receipt causal cycle and output-commit discovery.
...
| `PlanReview` | `dispatch-plan-review` | phase agent or control plane | No worker lease; no prior dispatch for this manifest | Accepted submitted manifest, verified pending-checkpoint completion, unchanged plan and governing contract, and matching deterministic dispatch identity and reviewer session | unchanged | Provider dispatch observed or launched and `plan-review-dispatched` recorded; accepted | portable provider-receipt checkpoint |
...
| `PlanReview` | `accept-plan-review` | phase agent | Matching fresh review session that did not author or revise the plan and did not receive its authoring conversation | Completed provider-receipt checkpoint, exact reviewer commit supplied as an untrusted locator, full `PLANS.md` conformance and clean marker from that commit's sole output file, session and provider rules pass, plan and governing-contract targets remain unchanged, and every process finding has one terminal disposition | `NeedsApproval` | Review accepted; completed | portable review verdict |
...
| Any nonterminal state | `record-gate-decision` | control plane | State-preserving; no lease requirement | `record-gate-decision` scope; complete canonical decision; no duplicate inputs or evidence; target and evidence commits exist | unchanged | `gate-decision-recorded`; completed | local |
...
`deriveCheckpoint(event, projection)` is pure and returns the exact `PendingCheckpointV1`. For `plan-review-dispatched`, it returns `kind: "provider-dispatch-receipt"`, the canonical provider-receipt bytes already stored in the event, and destination `{ kind: "repo-artifact", identity: dispatch_id, path: "docs/plans/reviews/receipts/<issue-uuid>/<dispatch-uuid>.json" }`. Both UUID path segments use canonical lowercase form; the path is fixed and create-only. Every other portable transition returns `kind: "issue-checkpoint-record"`; its bytes are canonical JSON for exactly `IssueCheckpointRecordV1`, and its destination is `{ kind: "issue-ref", identity: issue_id, path: "refs/issues/<issue-uuid>" }`. The payload digest is SHA-256 of `payload.bytes` and must equal `payload.digest`. No adapter may choose another path, media type, record shape, or byte representation.

`PortableCheckpointPort.observe` reads the one derived destination and returns the exact bounded checkpoint bytes, digest, and committed artifact target when present. `writeIfAbsent` receives the complete payload bytes. For `repo-artifact`, it creates only the derived path, commits that sole file on the planning branch, and returns an `ArtifactReferenceV1` with path, commit, digest, provider, and external ID. For `issue-ref`, it parses the exact `IssueCheckpointRecordV1`, applies it through the git-native issue writer without altering unrelated issue data, rereads the resulting ref, extracts that exact checkpoint record by originating event ID, and returns the record's canonical bytes plus the ref target. Matching existing bytes are success; different bytes at the same deterministic identity are `CHECKPOINT_CONFLICT`. `completeCheckpoint` independently compares read-back bytes and digest with the pending payload, requires a non-null target, constructs `ValidatedCheckpointEvidenceV1`, and passes it as the sole server-derived input to `complete-checkpoint`. The immutable `portable-checkpoint-verified` event stores that complete evidence. Replay restores the checkpoint target only from the event; for a dispatch receipt, it also copies the target into `ValidatedReviewDispatchV1.receipt_target` without calling `PortableCheckpointPort`.
...
The checked catalogs contain exactly these values:
    ...
    type AuthorityScopeV1 = "submit-plan-review" | "dispatch-plan-review" | "decide-plan" | "record-gate-decision" | "dispatch-work" | "mutate-work" | "heartbeat-lease" | "takeover-lease" | "release-lease" | "review-work" | "record-learn" | "integrate" | "verify-merge" | "pause-work" | "cancel-work" | "reconcile-sources" | "complete-portable-checkpoint" | "record-process-finding" | "dispose-process-finding";
...
| `dispatch-plan-review` | `dispatch_id`, `reviewer_session_id` |
| `reject-plan-review` | `review_session_id`, `plan`, `review_output`, `blocker_codes` |
| `accept-plan-review` | `review_session_id`, `reviewer_commit` |
...
The named nested values have these complete shapes. No command may substitute free-form JSON for them.
    ...
    interface CheckpointDestinationV1 { kind: "issue-ref" | "repo-artifact"; identity: string; path: RepoPath; }
    interface IssueCheckpointRecordV1 { schema_version: 1; project_id: Uuid; issue_id: Uuid; originating_event_id: Uuid; event_kind: EventKindV1; lifecycle_state: LifecycleStateV1; artifact_references: ArtifactReferenceV1[]; }
    type PortableCheckpointPayloadV1 =
      | { kind: "issue-checkpoint-record"; media_type: "application/vnd.mandem.issue-checkpoint+json"; bytes: Uint8Array; digest: Sha256 }
      | { kind: "provider-dispatch-receipt"; media_type: "application/vnd.mandem.provider-dispatch-receipt+json"; bytes: Uint8Array; digest: Sha256 };
    ...
    interface ReviewDispatchIntentV1 { dispatch_id: Uuid; reviewer: ReviewSessionIdentityV1; prompt_digest: Sha256; }
    ...
    interface ValidatedReviewDispatchV1 { intent: ReviewDispatchIntentV1; receipt: ProviderDispatchReceiptV1; receipt_digest: Sha256; receipt_target: ArtifactReferenceV1 | null; }
    interface BoundReviewManifestV1 { plan: PlanTargetV1; governing_contract: GoverningContractTargetV1; complete_sanitized_prompt: string; complete_prompt_digest: Sha256; dispatch_intent: ReviewDispatchIntentV1; reviewer_role: "clean-room-plan-reviewer"; challenge_lenses: ReviewChallengeLensV1[]; output_path: RepoPath; author_attestations: ArtifactReferenceV1[]; reviser_attestations: ArtifactReferenceV1[]; reviewer_attestation: ArtifactReferenceV1; risk_policy: ReviewerRiskPolicyV1; }
    interface ReviewEvidenceBundleV1 { submitted_manifest: ReviewManifestTargetV1; manifest_bytes: Uint8Array; parsed_manifest: BoundReviewManifestV1; dispatch: ValidatedReviewDispatchV1; provider_receipt_bytes: Uint8Array; parsed_provider_receipt: ProviderDispatchReceiptV1; output_bytes: Uint8Array; output_path: RepoPath; output_commit: GitSha; reviewer_commit: GitSha; write_set: ReviewWriteV1[]; session_attestations: SessionAttestationV1[]; }
    ...
    interface LifecyclePolicyStateV1 { plan: PlanTargetV1 | null; governing_contract: GoverningContractTargetV1 | null; submitted_review_manifest: ReviewManifestTargetV1 | null; review_dispatch: ValidatedReviewDispatchV1 | null; plan_review: ReviewDecisionV1 | null; approval: ApprovalTargetV1 | null; gates: GateDecisionV1[]; handoff: HandoffDecisionV1 | null; reviewer_risk_policy: ReviewerRiskPolicyV1 | null; }

Fields named `plan`, `governing_contract`, `planning_pull_request` or `pull_request`, `review_manifest`, `review_output`, `approval`, `gate`, and `workspace` use the matching shape above. Fields named `self_check`, `validation_evidence`, `provider_evidence`, `verification_evidence`, `reconciliation_evidence`, `source_evidence`, `operator_summary`, `resolution_artifacts`, `changed_artifacts`, `repair_artifacts`, `evidence_artifacts`, or `learn_handoff` use `ArtifactReferenceV1[]`. Fields named `iteration_commit`, `reviewer_commit`, `reviewed_head`, `approved_head`, or `merge_sha` use `GitSha`. `destination` uses `CheckpointDestinationV1`. Every `*_id` uses `Uuid` except `finding_id`, which uses `FindingId`. Every `*_at`, `observed_at`, and `*_expires_at` uses `UtcTimestamp`. `evidence_code` uses `ProcessFindingEvidenceCodeV1`; process-finding `reason_code` uses `ProcessFindingReasonCodeV1`; pause or cancel `reason_code` uses `PauseOrCancelReasonCodeV1`; takeover or release `reason_code` uses `LeaseChangeReasonV1`; `resolution_code` uses `ResolutionCodeV1`; and blocker, failure, or conflict codes use `ErrorCodeV1`. No code contains prose. Array and string limits come from Protocol and Durable Data Limits.
...
`complete_sanitized_prompt` is the exact prompt body delivered to the reviewer. It is NFC UTF-8 text with LF endings, no byte-order mark, no credential or personal-data fields, and exactly one trailing LF. It may be at most 128 KiB. `complete_prompt_digest` is lowercase SHA-256 of those exact bytes. The pre-dispatch manifest embeds only `ReviewDispatchIntentV1`, whose deterministic dispatch UUID, reviewer identity, and prompt digest are known before provider launch. Its prompt digest must equal `complete_prompt_digest`, and its reviewer identity must equal the reviewer attestation. The manifest never names or predicts a provider receipt.

The provider receipt is canonical JSON for exactly `ProviderDispatchReceiptV1`, followed by one LF and limited to 32 KiB. `dispatchPlanReview` derives it only from `ReviewDispatchPort`, validates its dispatch ID, reviewer session, prompt digest, provider, and model against the accepted manifest intent, and records the parsed receipt and digest in `plan-review-dispatched`. The same transaction creates a portable checkpoint whose deterministic destination is the round's receipt path. After checkpoint verification, replay sets `ValidatedReviewDispatchV1.receipt_target` to that exact path, commit, digest, provider, and external ID. Acceptance requires the non-null verified target. The evidence adapter reads only that state-bound target, returns its raw bytes, parses the closed schema, and independently hashes the bytes. Missing bytes return `ARTIFACT_MISSING`; target, byte, digest, schema, or field mismatch returns `ARTIFACT_STALE`. The canonical validated bundle digest covers manifest bytes, provider-receipt bytes, output bytes, sorted write records, and sorted attestation digests in that order.
...
`EventPayloadV1` uses `kind` as its discriminator. Except for the server-derived variants below, the transition table maps each lifecycle command to exactly one past-tense payload with `{ kind, from_state, to_state, command_payload, lease_change }`: `command_payload` is the complete accepted `CommandPayloadV1` variant, and `lease_change` is `null` unless the transition changes a lease, otherwise it is the complete resulting `LeaseSnapshotV1`. Portable transitions then emit `portable-checkpoint-requested` with exactly `{ kind, checkpoint_id, originating_event_id, payload, destination }`, where `payload` is the complete `PortableCheckpointPayloadV1`. An implementation may not flatten, omit, or add fields.

    type ServerDerivedLifecycleEventPayloadV1 =
      | { kind: "plan-review-dispatched"; from_state: "PlanReview"; to_state: "PlanReview"; command_payload: Extract<CommandPayloadV1, { kind: "dispatch-plan-review" }>; lease_change: null; dispatch: ValidatedReviewDispatchV1 }
...
The application, not the caller, supplies `dispatch`, `review_decision`, `revoked_integration_lease`, and `created_work_lease`. Their bytes are part of the event digest and append ledger. Replay reconstructs dispatch, accepted review, and lease replacement only from these event values even when external evidence disappears or changes. The state-preserving families use these exact forms:

    type StatePreservingEventPayloadV1 =
      | { kind: "gate-decision-recorded"; decision: GateDecisionV1 }
      | { kind: "portable-checkpoint-verified"; evidence: ValidatedCheckpointEvidenceV1 }
...
The control plane may accept `record-gate-decision` in any nonterminal state from a principal whose role is `control-plane` and whose scopes include `record-gate-decision`. It validates the complete decision, sorts `input_digests` and `evidence` canonically, rejects duplicates, and appends `gate-decision-recorded` without changing lifecycle state or requiring a portable checkpoint. The event replaces the current value for the same `gate_id`; a changed definition, input, target, outcome, evidence, or decision time therefore requires a new idempotency key and event. An exact command retry returns its stored receipt. Guards accept only the latest event for each gate ID and return `GATE_FAILED` for `failed` or `GATE_STALE` when the stored definition, inputs, target, or evidence digest does not match the guarded transition. Replay applies gate events in sequence and never reads a gate provider. U4 may later supply the producer and user-facing gate commands, but it must call this U2 ingestion command rather than write projection rows directly.

The finding-policy reducer derives `effect`; callers never submit it. `execution-deviation` and `no-reusable-change` set `approved_intent_changed` and `checkpoint_required` to `false`, keep `to_state` equal to `from_state`, set both invalidated objects to `null`, and use an empty gate-ID array. `issue-contract-gap`, `product-contract-gap`, and `operating-contract-gap` always set both booleans to `true`, set `to_state` to `NeedsPlanning`, copy the current review and approval into the invalidated fields, and copy all current gate IDs in lexical order. Missing required current values remain `null` or empty; replay applies exactly the recorded effect and removes each listed gate from the current gate projection. Supersession derives a new effect from the new disposition and current policy state. Only principals with `dispose-process-finding` scope may dispose or supersede; the lifecycle rows further restrict their roles.
...
A command receipt stores `idempotency_key`, `command_kind`, `canonical_payload_digest`, `correlation_id`, `result_bytes`, and `created_at`. A checkpoint record stores `checkpoint_id`, `originating_event_id`, payload kind, media type, exact payload bytes, payload digest, destination, `state: "pending" | "verified" | "conflict"`, read-back digest, verified artifact target, and the creating and verifying sequence values. Optional database columns represent absence only for fields that the wire schema marks nullable.
...
The public ports use these signatures:
    ...
    interface PendingCheckpointV1 { checkpoint_id: Uuid; originating_event_id: Uuid; payload: PortableCheckpointPayloadV1; destination: CheckpointDestinationV1; }
    interface ObservedCheckpointV1 { state: "absent" | "matching" | "conflicting"; destination: CheckpointDestinationV1; read_back_bytes: Uint8Array | null; read_back_digest: Sha256 | null; target: ArtifactReferenceV1 | null; }
    interface ValidatedCheckpointEvidenceV1 { checkpoint_id: Uuid; originating_event_id: Uuid; payload_digest: Sha256; destination: CheckpointDestinationV1; read_back_digest: Sha256; target: ArtifactReferenceV1; }
    ...
    interface RoutedItemProjectionV1 { finding_id: FindingId; kind: "process-finding"; origin: ProcessFindingOriginV1; affected_phase: PhaseV1; evidence_code: ProcessFindingEvidenceCodeV1; evidence_artifacts: ArtifactReferenceV1[]; disposition: ProcessFindingDispositionV1 | null; repair_artifacts: ArtifactReferenceV1[]; disposition_event_id: Uuid | null; supersedes_event_id: Uuid | null; unresolved: boolean; }
    interface CheckpointProjectionV1 { checkpoint_id: Uuid; originating_event_id: Uuid; payload: PortableCheckpointPayloadV1; destination: CheckpointDestinationV1; state: "pending" | "verified" | "conflict"; read_back_digest: Sha256 | null; target: ArtifactReferenceV1 | null; creating_sequence: string; verifying_sequence: string | null; }
    interface RebuiltProjectionsV1 { lifecycle: IssueProjectionV1; leases: LeaseSnapshotV1[]; gates: GateDecisionV1[]; routed_items: RoutedItemProjectionV1[]; checkpoints: CheckpointProjectionV1[]; lifecycle_digest: Sha256; leases_digest: Sha256; gates_digest: Sha256; routed_items_digest: Sha256; checkpoints_digest: Sha256; }
    interface VerifiedProjectionReplacementV1 { project_id: Uuid; issue_id: Uuid; expected_events_digest: Sha256; expected_last_sequence: string; rebuilt: RebuiltProjectionsV1; }
    interface LifecyclePolicyInputV1 { snapshot: IssueLedgerSnapshotV1; command: CommandPayloadV1; actor: ActorAttributionV1; command_id: Uuid; occurred_at: UtcTimestamp; validated_review_dispatch: ValidatedReviewDispatchV1 | null; validated_review_evidence: ValidatedReviewEvidenceV1 | null; validated_checkpoint_evidence: ValidatedCheckpointEvidenceV1 | null; }
...
`evaluateLifecycleCommand(input: LifecyclePolicyInputV1): LifecyclePolicyDecisionV1` is the only command reducer entry point. `reduceEventStreamV1(events: AsyncIterable<EventEnvelopeV1>): Promise<RebuiltProjectionsV1 | ProtocolErrorV1>` is the replay reducer. Both read no clock, database, Git, provider, or ambient working-tree state. Application code builds command input from the validated envelope and principal plus `EventStorePort.loadIssue`. Only `dispatch-plan-review` may carry `validated_review_dispatch`; only `accept-plan-review` may carry `validated_review_evidence`; only `complete-checkpoint` may carry `validated_checkpoint_evidence`; every other command requires all three to be `null`. Replay validates event order and digests, applies every recorded payload, and returns all five complete projection collections. It sorts leases by resource kind then lease UUID, gates by gate UUID, routed items by finding ID, and checkpoints by checkpoint UUID. It serializes each complete collection with the canonical JSON rules and hashes those exact bytes. It never repeats external I/O or reevaluates historical freshness or risk policy. Parsers, serializers, fixtures, port fakes, and both root barrels include every policy, lease, gate, routed-item, checkpoint, and rebuilt-projection value above.
...
`replaceProjections` first compares `expected_events_digest` and `expected_last_sequence` with the non-disposable append-ledger anchor. It then reserializes `rebuilt.lifecycle` and each canonically ordered collection, verifies all five supplied digests, and replaces all five live projection sets from those complete values in one transaction. A missing row, duplicate identity, wrong order, byte or digest mismatch, or concurrent anchor change returns `PROJECTION_MISMATCH` and preserves every live projection row.
    interface PortableCheckpointPort { observe(checkpoint: PendingCheckpointV1): Promise<ObservedCheckpointV1>; writeIfAbsent(checkpoint: PendingCheckpointV1): Promise<ObservedCheckpointV1>; }
    interface PlanContentPort { readExact(commit: GitSha, path: RepoPath): Promise<Uint8Array>; }
    interface ReviewDispatchObservationV1 { state: "absent" | "delivered"; receipt_bytes: Uint8Array | null; }
    interface ReviewDispatchPort { observe(dispatchId: Uuid): Promise<ReviewDispatchObservationV1>; dispatch(intent: ReviewDispatchIntentV1, promptBytes: Uint8Array): Promise<Uint8Array>; }
    interface ReviewEvidencePort { loadBoundEvidence(submittedManifest: ReviewManifestTargetV1, dispatch: ValidatedReviewDispatchV1, reviewerCommit: GitSha): Promise<ReviewEvidenceBundleV1>; }
...
`submit-plan-review` stores its exact pre-dispatch `review_manifest` in `LifecyclePolicyStateV1.submitted_review_manifest`. No provider launch is permitted before that transition and its portable checkpoint complete. `dispatchPlanReview` then loads the exact manifest bytes, validates the plan, governing contract, prompt, dispatch identity, reviewer, role, lenses, and risk policy, and calls `ReviewDispatchPort.observe(dispatch_id)` before any launch. A matching delivered receipt makes retry safe; `absent` permits one call to `dispatch(intent, promptBytes)`. The use case parses and validates the returned canonical receipt, then invokes `dispatch-plan-review` with the server-derived `ValidatedReviewDispatchV1`; the event and pending receipt checkpoint commit atomically. A response lost after provider delivery is recovered by observing the deterministic dispatch ID. A receipt created before manifest acceptance, for another ID, prompt, reviewer, provider, or model returns `ACTOR_ATTRIBUTION_MISMATCH` and cannot enter policy state. Until the receipt checkpoint is verified, review acceptance returns `CHECKPOINT_PENDING`.

`accept-plan-review` cannot select another manifest or receipt; its only untrusted locator is `reviewer_commit`. `executeCommand` reads the stored manifest and verified dispatch state and passes both plus that commit to `ReviewEvidencePort`. If any is absent, acceptance returns `ARTIFACT_MISSING` or `CHECKPOINT_PENDING` with `refresh-plan-review` or `complete-checkpoint`.

`ReviewEvidencePort` is application-owned and trust-bearing. Its Git-backed adapter reads the submitted manifest bytes at the exact commit, reads the dispatch state's exact provider-receipt target and bytes, reads every session-attestation and risk-limitation artifact named by the manifest, and reads output only from the caller-supplied exact `reviewer_commit`. It requires that commit to descend from both the manifest commit and receipt-target commit on the planning branch, sets `output_commit` and `reviewer_commit` to that same commit, reads only the manifest output path there, and obtains only that commit's parent-to-commit write set. It has no branch-head, first-descendant, latest-write, dispatch-record, or receipt fallback. `validateReviewEvidence(bundle, submittedManifest, currentPlan, currentContract): ValidatedReviewEvidenceV1 | ProtocolErrorV1` hashes `manifest_bytes`; requires the stored target and current plan and contract; canonicalizes and hashes the prompt; parses and hashes `provider_receipt_bytes` and compares its target and every field with dispatch state and reviewer attestation; requires the exact reviewer role and challenge lenses; parses and hashes `output_bytes`; requires `output_commit === reviewer_commit`, the required ancestry, and exactly one write equal to the output path and digest; resolves distinct session attestations; rejects inherited context; applies risk rules; and computes the canonical bundle digest. Terminal output, missing receipt, or missing marker returns `ARTIFACT_MISSING`; substituted target, receipt, prompt, role, lens, write, identity, context, non-descendant commit, or mismatched reviewer session returns `ACTOR_ATTRIBUTION_MISMATCH`; stale bytes, plan, contract, receipt, attestation, or availability evidence returns `ARTIFACT_STALE`; a parsed `changes-required` verdict returns `ARTIFACT_STALE` and `refresh-plan-review`. `executeCommand` passes only a clean validated value to the reducer and stores it in the server-derived accepted-review event.

`src/modules/runtime/index.ts` exports the envelopes, closed catalogs, parsers, serializers, digest functions, receipt/checkpoint values, and all eight port interfaces through the domain and application barrels. It does not export SQLite, filesystem, locking, migration, or composition implementations. `src/modules/execution/index.ts` exports the lifecycle catalog, reducer, freshness and lease values, routed-item policy, review dispatch and evidence validators, and command/checkpoint/replay use cases through its domain and application barrels. Its infrastructure barrel remains empty, and consumers never import either module through a subpath.
...
| Portable checkpoint payload | Closed variant with canonical bytes | 256 KiB bytes; one payload per transition | Requested event, outbox, checkpoint adapter, and verified event |
...
- **Test scenarios:**
  ...
  3a. A control-plane principal with `record-gate-decision` scope records the complete decision without changing state. Exact command retry returns stored bytes; a later decision for the same gate replaces only that gate in projection order. A failed or stale latest decision blocks its dependent transition, and replay rebuilds it without a provider read.
...
- **Files:** `src/modules/runtime/application/ports/event-store.ts`, `src/modules/runtime/application/ports/portable-checkpoint.ts`, `src/modules/runtime/application/ports/plan-content.ts`, `src/modules/runtime/application/ports/review-dispatch.ts`, `src/modules/runtime/application/ports/review-evidence.ts`, `src/modules/runtime/application/ports/command-principal.ts`, `src/modules/runtime/application/ports/database-supervisor.ts`, `src/modules/runtime/application/ports/clock.ts`, `src/modules/runtime/application/index.ts`, `src/modules/runtime/infrastructure/repositories/git-review-evidence.ts`, `src/modules/runtime/infrastructure/repositories/git-review-evidence.test.ts`, `src/modules/runtime/tests/fakes/event-store.ts`, `src/modules/runtime/tests/fakes/portable-checkpoint.ts`, `src/modules/runtime/tests/fakes/plan-content.ts`, `src/modules/runtime/tests/fakes/review-dispatch.ts`, `src/modules/runtime/tests/fakes/review-evidence.ts`, `src/modules/runtime/tests/fakes/command-principal.ts`, `src/modules/runtime/tests/fakes/database-supervisor.ts`, `src/modules/runtime/tests/fakes/clock.ts`, `src/modules/runtime/tests/fakes/index.ts`, `src/modules/execution/application/services/validate-review-evidence.ts`, `src/modules/execution/application/services/validate-review-evidence.test.ts`, `src/modules/execution/application/use-cases/dispatch-plan-review.ts`, `src/modules/execution/application/use-cases/dispatch-plan-review.test.ts`, `src/modules/execution/application/use-cases/execute-command.ts`, `src/modules/execution/application/use-cases/execute-command.test.ts`, `src/modules/execution/application/use-cases/complete-checkpoint.ts`, `src/modules/execution/application/use-cases/complete-checkpoint.test.ts`, `src/modules/execution/application/use-cases/rebuild-projections.ts`, `src/modules/execution/application/use-cases/rebuild-projections.test.ts`, `src/modules/execution/application/index.ts`.
...
- **Test scenarios:**
  ...
  8. Replay from a valid stream yields complete, canonically ordered `RebuiltProjectionsV1` values for lifecycle, lease, gate, routed-item, and checkpoint projections. Every collection digest and the append-ledger anchor match before one atomic replacement, and the rebuilt lifecycle exposes one next permitted action.
  ...
  10. Review evidence rejects terminal-only output, a substituted evidence artifact or manifest, absent, changed, noncanonical, oversized, or dispatch-mismatched complete prompt bytes, an absent or substituted post-dispatch receipt target, a reviewer-attested prompt-digest mismatch, changed reviewer role, missing challenge lens, extra writes, author or reviser self-review, inherited or unattested authoring context, same-provider/model reviser reuse, stale availability evidence, and a required-but-unused alternative. Output parsing rejects absent, repeated, malformed, non-final, and noncanonical verdict markers; a reviewer-authored `CHANGES_REQUIRED` marker can never accept. Valid evidence accepts and stores `required-and-used` and properly evidenced `unavailable-permitted` cases.
  ...
  12. Dispatch cannot call the provider before the pre-dispatch manifest checkpoint completes. Same-ID retry observes the original receipt without a second launch. A pre-manifest, different-ID, prompt, reviewer, provider, or model receipt is rejected. Acceptance rejects two plausible output commits, stale-round output, a non-descendant commit, and a supplied commit whose output or reviewer session disagrees with dispatch evidence.
  13. Stop after dispatch commits to SQLite but before the receipt artifact write. Restart observes the one derived receipt path, writes the exact outbox bytes, records the returned commit target in `ValidatedCheckpointEvidenceV1`, and permits acceptance. Matching preexisting bytes succeed; conflicting bytes at that path enter checkpoint conflict. Repeat for an issue-ref checkpoint. For each destination kind, first append a current gate through `record-gate-decision` and populate every other projection from declared events. Capture the canonical bytes for the live lifecycle, lease, gate, routed-item, and checkpoint projections. Delete every row from all five disposable projection sets, configure `PortableCheckpointPort` to fail if replay calls it, rebuild only from the event stream, and require `replaceProjections` to receive and write byte-identical complete values for every captured projection before it replaces the live rows. For the receipt case, then submit review acceptance and prove that it uses the rebuilt non-null receipt target.
...
- **Concrete red/green proof:** Create the eight port interfaces and their in-memory fakes first. Then add tests named `dispatches only after the manifest checkpoint`, `recovers a lost dispatch response by identity`, `returns stored bytes after a lost command response`, `blocks a successor while checkpoint evidence is pending`, `observes before writing a checkpoint`, `records and replays the latest gate decision`, `reads plan bytes from the approved commit`, `derives review acceptance from one exact reviewer commit`, `parses only the reviewer-authored final verdict marker`, `rebuilds every projection after a repo-artifact checkpoint`, `rebuilds every projection after an issue-ref checkpoint`, and `replaces projections only after anchor verification`. The two checkpoint rebuild tests must use declared events to populate lifecycle, lease, gate, routed-item, and checkpoint rows, capture each row's canonical bytes, delete all five projection sets, make `PortableCheckpointPort` throw on any call, replay the ledger, assert that `replaceProjections` receives every complete typed value and matching collection digest, and compare every stored row byte-for-byte after replacement. The repo-artifact test must then accept the review through the rebuilt receipt target. Add every dispatch-order, prompt, receipt, marker, output-commit ambiguity, terminal-only, extra-write, author, reviser, inherited-context, risk-availability, and stale-evidence case above to the dispatch and validation tests; exercise ancestry and decoy commits against a disposable Git repository. Run `bunx vitest run src/modules/execution/application src/modules/runtime/infrastructure/repositories/git-review-evidence.test.ts`; it must fail because `GitReviewEvidenceRepository`, `dispatchPlanReview`, `parseReviewOutputVerdict`, `validateReviewEvidence`, `executeCommand`, `completeCheckpoint`, `reduceEventStreamV1`, and `rebuildProjections` are absent. Implement those named exports, re-export application contracts through their root barrels without exporting the Git adapter, and rerun the command until every test passes. Run `bun run typecheck` afterward to prove each fake and adapter satisfies the public port without a cast.
...
- [x] (2026-08-04) Repaired round nine's two P1 findings with a pre-dispatch manifest, a post-manifest dispatch use case, and one caller-supplied exact reviewer commit whose ancestry, bytes, write set, session, and verdict are independently validated.
- [x] (2026-08-04) Repaired round ten's final P1 by storing exact checkpoint payload bytes, deriving every destination, returning committed targets, and specifying receipt and issue-ref interruption tests.
...
- Decision: Carry provider-receipt bytes through the evidence port instead of trusting its artifact reference.
  ...
  Date/Author: 2026-08-04 / Codex, responding to clean-room review
- Decision: Split review setup, provider dispatch, and verdict acceptance into three ordered actions.
  Rationale: A provider receipt cannot exist before dispatch, while safe dispatch requires an accepted immutable manifest. A separate idempotent dispatch action preserves causal order, and an exact reviewer commit removes output discovery.
  Date/Author: 2026-08-04 / Codex, responding to clean-room review
- Decision: Store exact portable-checkpoint payload bytes in the outbox and derive destinations in pure policy.
  Rationale: A digest cannot tell an adapter what to write, and adapter-chosen paths make recovery nondeterministic. Exact bytes and one derived destination make observe-before-write recovery replayable.
  Date/Author: 2026-08-04 / Codex, responding to clean-room review
- Decision: Store the verified checkpoint target in the immutable verification event.
  Rationale: A projection-only target disappears during rebuild. The event must carry the exact target returned by trusted read-back so replay never consults mutable external state.
  Date/Author: 2026-08-04 / Codex, responding to clean-room review
- Decision: Test complete projection rebuild separately for both portable checkpoint destinations.
  Rationale: A focused checkpoint replay can pass while stale lease, gate, or routed-item rows remain. Each test must delete all disposable projections and compare every rebuilt value before replacement.
  Date/Author: 2026-08-04 / Codex, responding to clean-room review
- Decision: Carry complete typed projection collections through one verified replacement value.
  Rationale: Collection digests can detect change but cannot supply deleted rows. The application must give the adapter every reconstructed value before the adapter can replace all projections atomically.
  Date/Author: 2026-08-04 / Codex, responding to clean-room review
...
Two-stage-dispatch revision note (2026-08-04): Repaired round nine's causal-order and output-selection gaps. The committed manifest now contains only deterministic pre-dispatch intent. A separate observe-before-launch use case records the provider receipt and portable checkpoint after manifest acceptance. Review acceptance then reads one exact caller-supplied reviewer commit, proves its ancestry and sole write, and derives all output facts from its bytes. This revision requires another bound clean-room verdict.

Concrete-checkpoint revision note (2026-08-04): Repaired round ten's checkpoint payload and destination gap. Pending checkpoints now carry exact bounded bytes, media type, and digest. Pure policy derives the sole receipt-artifact or issue-ref destination; the port writes or extracts those exact bytes and returns the committed target; replay records the verified receipt target for later ancestry checks. This revision requires another bound clean-room verdict.

Verified-target-event revision note (2026-08-04): Repaired round eleven's remaining checkpoint replay gap. `completeCheckpoint` now supplies a closed server-validated evidence value with the exact committed target, and `portable-checkpoint-verified` stores it unchanged. Event-only replay restores checkpoint state and dispatch receipt ancestry without calling the checkpoint port. This revision requires another bound clean-room verdict.
