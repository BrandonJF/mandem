...
The public ports use these signatures:
    ...
    interface LifecyclePolicyInputV1 { snapshot: IssueLedgerSnapshotV1; command: CommandPayloadV1; actor: ActorAttributionV1; command_id: Uuid; occurred_at: UtcTimestamp; validated_review_dispatch: ValidatedReviewDispatchV1 | null; validated_review_evidence: ValidatedReviewEvidenceV1 | null; }
...
`evaluateLifecycleCommand(input: LifecyclePolicyInputV1): LifecyclePolicyDecisionV1` is the only lifecycle reducer entry point. It reads no clock, database, Git, provider, or ambient working-tree state. Application code builds the input from the validated envelope and principal plus `EventStorePort.loadIssue`. Only `dispatch-plan-review` may carry `validated_review_dispatch`; only `accept-plan-review` may carry `validated_review_evidence`; every other command requires both to be `null`. Replay applies recorded event payloads and never repeats provider dispatch or reevaluates historical freshness or risk policy. Parsers, serializers, fixtures, and both root barrels include every policy and lease value above.
...
- **Test scenarios:**
  ...
  13. Stop after dispatch commits to SQLite but before the receipt artifact write. Restart observes the one derived receipt path, writes the exact outbox bytes, records the returned commit target, and permits acceptance. Matching preexisting bytes succeed; conflicting bytes at that path enter checkpoint conflict. The same test proves an issue-ref checkpoint extracts and compares its exact canonical record.
...
Planning PR #37 now contains ten authoritative clean-room review rounds and the repairs they required. Earlier rounds closed protocol, process-finding, milestone, SQLite, policy-state, lease, disposition, evidence-boundary, release-path, exact-byte, accepted-event, merge-repair, living-section, prompt, verdict, dispatch, receipt, causal-order, and output-selection findings. Round ten found that portable checkpoints still lacked exact bytes and derived destinations; the current revision adds both, plus committed-target read-back and interruption tests for receipt artifacts and issue refs. No clean verdict, operator approval, implementation, or runtime evidence exists yet. The next permitted action is to bind this exact revision and current `PLANS.md` in another fresh clean-room review, then repair any finding or present a clean reviewed commit for standalone `execute-plan` approval. Implementation remains unauthorized.
